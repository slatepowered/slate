...
import com.orbyfied.slate.loader.PriorityURLClassLoader;
import com.orbyfied.slate.util.json.Json;
...
public final class ApplicationBootstrap {
  ...
  public static final String ROOT_MODULE_INFO_RESOURCE = "META-INF/slate-module-metadata/auto.root.json";
  public static final String BUNDLED_LIBRARIES_RESOURCE = "META-INF/bundled-libs/";

  /* Bootstrapper Settings */
  static final boolean DEBUG = true;
  static final Path CACHE_PATH = Path.of(System.getProperty("user.home"), ".slate", "boot.cache");
  ...
  public static void main(String[] args) throws Throwable {
    log("Bootstrapping slate module");
    ...
    try (JarFile jar = new JarFile(selfJarPath.toFile())) {
      ...
      while (entries.hasMoreElements()) {
        ...
        if (name.equals(ROOT_MODULE_INFO_RESOURCE)) {
          ...
          try (InputStream is = jar.getInputStream(entry)) {
            ...
            JsonObject metadata = GSON.fromJson(new BufferedReader(new InputStreamReader(is)), JsonObject.class);
            appInfo.loadRootMetadata(metadata);
          }

          checklist.completed(ROOT_MODULE_INFO_RESOURCE);
          log("Completed parse of root module info, identified name(" + appInfo.name + ") group(" + appInfo.group + ") version(" + appInfo.version + ")");
        ...
        if (name.startsWith(BUNDLED_LIBRARIES_RESOURCE)) {
          ...
          appInfo.extractedBundledLibraries.add(output.toUri().toURL());
      ...
      log("Creating application class loader with " + appInfo.extractedBundledLibraries.size() + " initial libraries");
      URL[] urls = appInfo.extractedBundledLibraries.toArray(new URL[0]);
