...
import slate.build.task.GeneratePackageMetadataTask
...
class SlateBuildPlugin implements Plugin<Project> {
  ...
  void apply(Project project) {
    ...
    project.afterEvaluate {
      // check for required java plugin
      if (!project.plugins.hasPlugin("java")) {
        throw new IllegalArgumentException("slate-build: Project `" + project.name + "` does not have the 'java' plugin applied, the java plugin is required for " +
            "all package builds, including those without classes");
      }
      ...
      for (PackageComponent component : packageDef.components()) {
        ...
        component.sourceSet.set(sourceSet)
    ...
    project.afterEvaluate {
      ...
      for (def comp : packageDef.components()) {
        def generateTask = comp.moduleMetadataTask =
            project.tasks.register(
                comp.sourceSet.get().getTaskName("generate", "ComponentMetadata"),
        ...
        project.tasks.withType(ProcessResources).named(comp.sourceSet.get().getProcessResourcesTaskName()) {
        ...
        project.tasks.withType(Jar).named(comp.sourceSet.get().getJarTaskName()) {
          if (!comp.isMain()) {
        ...
        (comp.jarTask = project.tasks.withType(Jar).named(comp.sourceSet.get().getJarTaskName())).configure {
          if (comp.code) {
            ...
            bundledConf.resolvedConfiguration.resolvedArtifacts.each { artifact ->
              from(artifact.file) {
                rename {
                  "META-INF/bundled-libs/${artifact.moduleVersion.id.group}." +
                      "${artifact.name}@" +
                      "${artifact.moduleVersion.id.version}.jar"
                }
              }
            }
          }
      ...
      for (def comp : packageDef.components()) {
        ...
        if (comp.isApplication()) {
          ...
          project.tasks.register(comp.sourceSet.get().getTaskName("run", "Jar"), Exec) {
            group = "application"
            description = "Builds and runs the generated jar of the `" + comp.name + "` component"

            dependsOn(comp.sourceSet.get().getJarTaskName())

            doFirst {
              def jarTask = project.tasks.named(comp.sourceSet.get().getJarTaskName()).get()
      ...
      def packageJarTask = project.tasks.register("packageJar", Jar) {
        ...
        it.from(metadataTask.get().outputFile)

        // copy component jars and metadata files
        for (def comp : packageDef.components()) {
          it.dependsOn(comp.jarTask)

          it.from(comp.jarTask.flatMap { it.archiveFile }) {
            rename { comp.embeddedJarName() }
          }

          it.from(comp.moduleMetadataTask.get().componentManifestFile) {
